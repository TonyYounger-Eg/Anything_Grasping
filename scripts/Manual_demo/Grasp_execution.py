...
# dis_left 0.503000020980835 m
touch_left = np.array([0.00058, 0.0638, 0.419, 1])
# dis_right 0.4970000088214874 m
touch_right = np.array([0.03125, 0.03844, 0.422, 1])
touch_angle = -0.46365
...
def kinovagrasp_callback(pose_data):
    ...
    touch_center_after_eyeonhand = np.matmul(eye_on_hand_matrix, touch_center) + np.array([0, 0, 0+0.03, 0])
...
if __name__ == '__main__':
    ...
    subscriber = rospy.Subscriber("/j2n6s300_driver/out/tool_pose", PoseStamped, kinovagrasp_callback)  # 订阅KINOVA当前姿态的轨迹。
    rospy.spin()
