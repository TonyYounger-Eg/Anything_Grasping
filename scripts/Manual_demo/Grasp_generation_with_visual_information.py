...
#      Author: Tex Yan Liu
#
# description: 在这个文件中调整抓取坐标，开始尝试输出抓取姿态进行抓取，抓取需要借鉴到有限状态机的过程
...
from grasp_wrench_2d_v5 import grasp_wrench_2d
import rospy
...
def get_3d_point_open3d(pointcloud_tmp_depth_rs, x, y):
    get_3d_point_open3d_tmp_distance = pointcloud_tmp_depth_rs[y, x]
    print("open3d获得的深度信息", get_3d_point_open3d_tmp_distance)
    # 将像素坐标转换为相机坐标
    camera_point = o3d.geometry.Image([x, y, get_3d_point_open3d_tmp_distance])
    get_3d_point_open3d_tmp_point = o3d.geometry.PointCloud.create_from_depth_image(
        depth=camera_point, intrinsic=o3d.camera.PinholeCameraIntrinsic(
            640, 480, 605.471,  604.936, 327.157, 247.894)
    )

    return get_3d_point_open3d_tmp_distance, get_3d_point_open3d_tmp_point
...
def vector_to_quaternion_1(v):
    ...
    return np.array([q0, q1, q2, q3])  # qw qx qy qz
        ...
        while True:
            ...
            if not seg_input_stop_laser and len(np.array(seg_input_label_laser_tmp)) > 100:
                ...
                if decision == "y" or decision == "n":
                    ...
                    local_pcl_points = get_pointcloud(img_color, local_depth)  # 保存局部点云
                    kmean_lines, local_min_obb, tmp_a, tmp_b = local_pointcloud_process(local_pcl_points, k)
                    structure_kmean_lines.append(kmean_lines)
                    global_pcl_points = get_pointcloud(img_color, img_depth)  # 获取全局点云
                    structure_kmean_lines_for_def = copy.deepcopy(structure_kmean_lines)  # 深度拷贝，防止影响原值
                    global_pointcloud_process(global_pcl_points, k, structure_kmean_lines_for_def, local_min_obb, tmp_a, tmp_b)
            ...
            # 2.鼠标交互, 鼠标中点左键就是前景点1，鼠标中点右键就是后景点标签0。
                ...
                if decision == "y":
                    ...
                    求解机器人的预调整姿态
                    """
                    attention_t = camera_coordinate  # 提示点相对于相机坐标系原点的坐标 （圆心）
                    ...
                    axis_pcd_1 = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2, origin=attention_t_2)  # 坐标轴1
                    axis_pcd_2 = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2, origin=attention_t_2)  # 坐标轴2
                    ...
                    new_axis_x = np.cross(camera_coordinate/attention_mol, new_axis_z)
                    new_axis_x = new_axis_x / np.linalg.norm(new_axis_x)
                    # print("new_axis_x", new_axis_x)
                    # print("原点到圆心向量的模", np.linalg.norm(camera_coordinate/attention_mol))
                    # print("new_axis_x的模", np.linalg.norm(new_axis_x))
                    ...
                    attention_normal_line_set.points = o3d.utility.Vector3dVector(np.array([camera_coordinate, camera_coordinate - 0.5 * attention_normals])) # +号是冲外，-号是冲里
                    attention_normal_line_set.lines = o3d.utility.Vector2iVector(np.array([[0, 1]]))

                    o3d.visualization.draw_geometries([lin_shi_ceshi_o3d_vs_pc, attention_normal_line_set, axis_pcd, axis_pcd_1, axis_pcd_2]) # , point_show_normal=True)
                    ...
                    kmean_lines, local_min_obb, tmp_a, tmp_b = local_pointcloud_process(local_pcl_points, k)
                    structure_kmean_lines.append(kmean_lines)
                    # print("structure_kmean_lines", structure_kmean_lines)
                    structure_kmean_lines_for_def = copy.deepcopy(structure_kmean_lines)  # 深度拷贝，防止影响原值
                    global_pcl_points = get_pointcloud(img_color, img_depth)  # 获取并保存全局点云图像
                    global_pointcloud_process(global_pcl_points, k, structure_kmean_lines_for_def, local_min_obb, tmp_a, tmp_b)
            ...
            if len(seg_tmp_masks) != 0:
                ...
                # boundary = np.loadtxt("grasp_stability/txt/boundary.txt")
                # center = np.loadtxt("grasp_stability/txt/center.txt")
                ...
                if len(S_optimal) != 0:
                    ...
                    print("左侧接触像素点：", S_optimal[0])
                    print("右侧接触像素点：", S_optimal[1])
                    # cv2.arrowedLine(img_color, S_optimal[0] + S_rotate_degree, S_optimal[0], color=(0, 255, 0), thickness=2, tipLength=0.3)
                    dis_left, touch_left = get_3d_camera_coordinate([S_optimal[0][0]+15, S_optimal[0][1]],   # 先加后减能让左点更靠里
                                                                    aligned_depth_frame,
                                                                    depth_intrin)
                ...
                # kinova_rxryrz = np.array([0, 0, S_rotate_degree])   # 收集相机坐标系下应变换的角度
            ...
            cv2_vertices_lines_index = [[0, 1], [0, 2], [1, 3], [2, 3], [0, 4], [1, 5], [2, 6], [3, 7], [4, 5], [4, 6], [5, 7], [6, 7]]
            #k_instrin_in = [[605.471, 0.0, 327.157], [0.0, 604.936, 247.894], [0.0, 0.0, 1.0]]  # 相机内参
            #print("len(structure_kmean_lines)", len(structure_kmean_lines))
            for l in range(len(structure_kmean_lines)):
                cv2_3d_vertices = np.asarray(structure_kmean_lines[l].points)  # 提取所有的顶点
                #print("cv2_3d_vertices", cv2_3d_vertices)
                cv2_3d_vertices_z = np.array([cv2_3d_vertices[:, -1]])  # 提取所有z值，！！！！这个z的值不对，z的值应该等于[x,y]等值的点云的z
                #print("cv2_3d_vertices_z", cv2_3d_vertices_z)
                cv2_3d_vertices = cv2_3d_vertices / cv2_3d_vertices_z.T   # 除以所有z值
                #print("cv2_3d_vertices", cv2_3d_vertices)
                cv2_2d_vertices = np.matmul(k_instrin_in, cv2_3d_vertices.T)   # 乘内参
                #print("cv2_2d_vertices内参后", cv2_2d_vertices)
                cv2_2d_vertices = cv2_2d_vertices[0:2, :].astype(int)  # 取前两行
                cv2_2d_vertices = cv2_2d_vertices.T   # -----写到了这里
                #print("cv2_2d_vertices", cv2_2d_vertices)
                for m in range(len(cv2_vertices_lines_index)):
                    cv2_2d_vertices_index = cv2_vertices_lines_index[m]
                    # cv2.line(img_color,
                    #          (cv2_2d_vertices[cv2_2d_vertices_index[0]][0], cv2_2d_vertices[cv2_2d_vertices_index[0]][1]),
                    #          (cv2_2d_vertices[cv2_2d_vertices_index[1]][0], cv2_2d_vertices[cv2_2d_vertices_index[1]][1]),
                    #          (0, 0, 255), 1, 16)

            if k != 0:  # 如果是第一次，则不需要局部点云配准，否则需要进行局部配准
                A = 1

            # cv2.imshow("Scene", np.hstack((img_color, depth_mapped_image)))  # 展示彩色图像和深度图像
