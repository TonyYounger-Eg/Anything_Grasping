...
from grasp_stability.grasp_wrench_2d_v3 import grasp_wrench_2d
    ...
    try:
        while True:
            ...
            if len(seg_tmp_masks) != 0:
                ...
                cv2.line(img_color, S_optimal[0], S_optimal[1], (255, 0, 0), 1, cv2.LINE_AA)

                dis_left, touch_left = get_3d_camera_coordinate([S_optimal[0][0], S_optimal[0][1]],
                                                                  aligned_depth_frame,
                                                                  depth_intrin)
                dis_right, touch_right = get_3d_camera_coordinate([S_optimal[1][0], S_optimal[1][1]],
                                                                  aligned_depth_frame,
                                                                  depth_intrin)
                camera_xyz = (touch_left + touch_right) / 2  # 收集相机坐标系下实际坐标点的xyz坐标
            ...
            for l in range(len(structure_kmean_lines)):
                ...
                cv2_2d_vertices = cv2_2d_vertices.T   # -----写到了这    global subscriber
