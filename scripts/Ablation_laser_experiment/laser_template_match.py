...
if __name__ == '__main__':
    ...
    img_template = cv2.imread("template_laser_cut.png", 0)
    ...
    try:
        while True:
            ...
            text = "laser"
            for pt in zip(*loc[::-1]):
                cv2.putText(img_color, text, (pt[0]-5, pt[1]-5),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (180, 105, 255), 1)
                cv2.rectangle(img_color, pt, (pt[0] + w, pt[1] + h), (180, 105, 255), 1)

            cv2.imshow("Scene", img_color)  # 展示图像
            # cv2.imwrite("template_laser_origin.png", img_color)   # 保存模板源图
