...
#      Author: Tex Yan Liu
#
# description: Ros node for the module of interactive_prompt，用于测试没有预姿态调整的消融实验
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='../yolov5/yolov5s_laser.pt', help='model.pt path(s)')
        ...
        while True:
            ...
            if (seg_input_stop_mouse and len(np.array(seg_input_label_mouse)) > 0) or (laser_callback_status == "laser_back_is_ok"):
                ...
                if decision == "y":
                    ...
                    print(prompt_info)
                    '''
                    没有预定义姿态调整的话，就不用发布了，注释掉，然后直接将这个节点的回复定义为ok
                    '''
                    pose_adjustment_status = "ok"
            ...
            if pose_adjustment_status == "ok":    # ！！！！！需要加一些信息，看看center在这个过程中的保持程度
                ...
                time.sleep(1)
                print("进入SAM分割阶段")
                input_point = [[int(new_attention_pixel[0]), int(new_attention_pixel[0])]]
            ...
            if len(seg_tmp_masks) != 0:
                ...
                if pose_adjustment_status == "ok" and len(S_optimal) != 0:
                    ...
                    dis_left, touch_left = get_3d_camera_coordinate([S_optimal[0][0] + 20, S_optimal[0][1]],
                                                                    # 先加后减能让左点更靠里
                                                                    aligned_depth_frame,
                                                                    depth_intrin)
                    # print("dis_left", dis_left, "m")
                    print("左侧接触实际三维点", touch_left, "m")
                    dis_right, touch_right = get_3d_camera_coordinate([S_optimal[1][0] - 20, S_optimal[1][1]],
                                                                      aligned_depth_frame,
                                                                      depth_intrin)
